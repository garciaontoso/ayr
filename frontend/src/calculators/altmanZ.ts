...
import type { AltmanResult, FinancialsYear } from '../types';

export function calcAltmanZ(data: FinancialsYear | null | undefined, mktCap: number): AltmanResult {
  ...
  const workingCap = (data.cash||0) - ((data.totalDebt||0) * 0.3);
  ...
  const zone: AltmanResult['zone'] = z > 2.99 ? "Segura" : z > 1.81 ? "Gris" : "Peligro";
