              ...
              {sortedRows.map((g,i) => {
                ...
                return (
                  <tr key={g.id||i} id={g.id ? `gasto-${g.id}` : undefined} style={{background:i%2?"var(--row-alt)":"transparent",opacity:g.secreto?.5:1}}
                    ...
                    <td style={{padding:"5px 10px",fontFamily:"var(--fm)",color:"var(--text-secondary)",borderBottom:"1px solid var(--subtle-bg)",maxWidth:200,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{g.cat}{g.secreto?<span style={{fontSize:7,marginLeft:4,padding:"1px 4px",borderRadius:3,background:"rgba(99,102,241,.08)",color:"#6366f1",verticalAlign:"middle"}}>🔒</span>:""}{g.tipo==="china"?<><span style={{fontSize:7,marginLeft:4,padding:"1px 4px",borderRadius:3,background:"rgba(239,68,68,.08)",color:"#ef4444",verticalAlign:"middle"}}>🇨🇳 CHINA</span><button onClick={async(e)=>{e.stopPropagation();try{await fetch(`${API_URL}/api/gastos/${g.id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({china_obligatorio:!g.chinaOblig})});g.chinaOblig=!g.chinaOblig;e.currentTarget.textContent=g.chinaOblig?"OBLIG":"vol";}catch{}}} title={g.chinaOblig?"Obligatorio por vivir en China — click para marcar voluntario":"Voluntario — click para marcar obligatorio"} style={{fontSize:7,marginLeft:3,padding:"1px 5px",borderRadius:3,border:`1px solid ${g.chinaOblig?"rgba(239,68,68,.4)":"rgba(255,255,255,.1)"}`,background:g.chinaOblig?"rgba(239,68,68,.15)":"transparent",color:g.chinaOblig?"#ef4444":"var(--text-tertiary)",cursor:"pointer",verticalAlign:"middle",fontWeight:g.chinaOblig?700:500}}>{g.chinaOblig?"OBLIG":"vol"}</button></>:g.tipo==="extra"?<span style={{fontSize:7,marginLeft:4,padding:"1px 4px",borderRadius:3,background:"rgba(168,85,247,.08)",color:"#a855f7",verticalAlign:"middle"}}>EXTRA</span>:""}{g.recur?<span style={{fontSize:7,marginLeft:3,padding:"1px 4px",borderRadius:3,background:"rgba(255,159,10,.08)",color:"var(--orange)",verticalAlign:"middle"}}>REC</span>:""}</td>
