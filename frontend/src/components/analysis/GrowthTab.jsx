        ...
        <Card style={{overflowX:"auto",padding:0}}>
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:11.5}}>
            <thead><tr>
              ...
              {["3a","5a","10a"].map(p=><th key={p} style={{padding:"10px 12px",textAlign:"center",color:"var(--text-secondary)",fontWeight:600,borderBottom:"2px solid var(--table-border)",minWidth:90,fontFamily:"var(--fm)",fontSize:10}}>CAGR {p}</th>)}
              {YEARS.slice(0,8).map(y=><th key={y} style={{padding:"10px 6px",textAlign:"right",color:"var(--text-secondary)",fontWeight:600,borderBottom:"2px solid var(--table-border)",minWidth:72,fontFamily:"var(--fm)",fontSize:10}}>{y}</th>)}
            </tr></thead>
            <tbody>{metrics.map((m,i)=>(
              <tr key={m.k} style={{background:i%2?"var(--row-alt)":"transparent"}}>
                ...
                {[3,5,10].map(p=>{
                  ...
                  return <td key={p} style={{padding:"7px 12px",textAlign:"center",borderBottom:"1px solid var(--table-border)"}}>
                    <span style={{fontSize:13,fontWeight:700,color,fontFamily:"var(--fm)"}}>{fP(c)}</span>
                  </td>;
                })}
                {YEARS.slice(0,8).map(y=>{
                  const v = m.fn(y);
                  return <td key={y} style={{padding:"7px 6px",textAlign:"right",color:n(v)!=null&&v<0?"var(--red)":"var(--text-primary)",borderBottom:"1px solid var(--table-border)",fontFamily:"var(--fm)"}}>{m.k==="revenue"||m.k==="netIncome"?fM(v):fC(v)}</td>;
                })}
              </tr>
            ))}</tbody>
          </table>
